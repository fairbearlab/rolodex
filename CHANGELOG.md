...
* **Scoring rules for sparse contacts.** The linear score is still used for ranking, but rules now sit on top of the tier thresholds: an identical name (equal after normalization; a nickname is similarity, not identity) plus a shared phone, email or birthday is `auto_merge`; a near-identical name (similarity >= 0.95) on its own is floored at `review` instead of `distinct`; and two well-formed birthdays that disagree cap any pair at `review`, so a parent and child sharing a household phone are never merged unseen. Previously "same name + same phone" scored 0.65 and always needed a human, and "same name, nothing else" (0.40) was silently dropped. On the author's exports this moves auto-merges from 7 to 27 and the review queue from 4 to 42 clusters, out of 559 candidate pairs. Nickname pairs with a shared identifier (Bob/Robert Smith + same email) score 0.65 and are reviewed: a nickname is not an identity — Alex is Alexandra as well as Alexander, Sam is Samantha, Pat is Patricia — and one household phone is exactly what joins those pairs.
...
* **Same-name pairs are reviewed as pairs.** A pair that is in review on its name alone is a prompt for a human, not a link between people, so the merger no longer chains clusters through it. Before, union-find collapsed everyone sharing a common name into one review card — six unrelated "David Lee"s with six phones and six emails became one six-member cluster whose single merge action deleted five people. Near-name-only edges now pair two otherwise unattached contacts (most similar first, cross-source before same-source); a third namesake stays distinct instead of being stacked onto a cluster it has no tie to. Edges with a shared identifier chain as before. A same-name edge that is not applied is not dropped silently either: `report.json` lists it under `deferred` (with `deferred_count` in the summary) naming both sides, and the pipeline says how many such pairs were kept as separate people — before, the namesake went to `merged.vcf` on its own with no card, no cluster entry and no warning while the run had just printed it as a review pair.
...
* **Compact review card threshold** lowered from 0.78 to 0.60. Pairs at 0.60+ have a shared identifier and get the one-glance card; near-name-only pairs get the full diff. The compact card now shows the email or phone that actually matched on both sides (marked ✓, with a `+N` count of values it is not showing) instead of the first one, the birthdays, and a one-line score breakdown — a pair sharing its second email used to display two different addresses under "Match: email" with nothing to say which had matched. Pairs held in review by a birthday conflict can score up to 1.00 and always get the full diff, which is the only view that shows the birthdays.
* `rolodex merge` defaults `--review` to `review.vcf` next to `--out` instead of the current directory.
* The nickname table feeds name similarity only. Removing `liam`, `jack`, `jamie`, `leo` and `harry` (standalone given names) had kept Jack/John off the identical-name rule at the cost of the review floor: Jack Smith and John Smith on one phone scored 0.578 and were written out as two people with no card. Identity now requires literally equal given names, so the five are back for similarity and reach review. Differing middle names or generational suffixes (John A. Smith / John B. Smith, John Smith Jr. / Sr.) are not an identical name either.
